    ...
    rows.forEach(row => {
        ...
        headers.forEach((header, index) => {
            ...
            if (typeof value === 'string') {
                ...
                const numericCandidate = raw.replace(/,/g, ''); // Eliminar comas para números como "1,234.56"
                if (/^[+-]?\d+(?:\.\d+)?$/.test(numericCandidate)) {
                    const numValue = parseFloat(numericCandidate);
                    value = isNaN(numValue) ? raw : numValue;
                } else {
                    value = raw;
                }
            }
